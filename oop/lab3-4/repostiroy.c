...
void updateProductFromRepository(Repository *repository, Product *product, int id)
{
    int i;
    for (i = 0; i <= repository->length; i++) {
        if (repository->array[i]->id == id)
            updateProduct(repository->array[i], product);
    }
}
...
/*
 * Will delete a specific product from the repository using an id.
 ...
 */
int deleteProductFromRepository(Repository *repo, int id)
{
    int ok = 0;
    if (repo->length == 0)
        return -2;
    for (int i = 0; i < repo->length; i++) {
        if (repo->array[i]->id == id) {
            deleteProduct(repo->array[i]);
            ok = 1;
        }
        if (ok) {
